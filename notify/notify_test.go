...
func TestDedupStageNeedsUpdate(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			firingAlerts: map[uint64]struct{}{1: struct{}{}, 2: struct{}{}, 3: struct{}{}},
			resErr:       true,
		}, {
			entry: &nflogpb.Entry{
				FiringAlerts: []uint64{1, 2, 3},
...
func TestDedupStage(t *testing.T) {
	...
	s.nflog = &testNflog{
		...
		qres: []*nflogpb.Entry{
			...
			{FiringAlerts: []uint64{uint64(1), uint64(2), uint64(3)}},
		},
	}
	ctx, res, err = s.Exec(ctx, alerts...)
	...
	s.nflog = &testNflog{
		...
		qres: []*nflogpb.Entry{
			{
				FiringAlerts: []uint64{uint64(1), uint64(2), uint64(3), uint64(4)},
				Timestamp:    mustTimestampProto(now),
			},
		},
	}
...
func TestSetNotifiesStage(t *testing.T) {
	...
	now := utcNow()
	alert := &types.Alert{Alert: model.Alert{EndsAt: now.Add(-1 * time.Minute)}}
	alerts = []*types.Alert{alert, alert, alert}
	i = 0
	tnflog.logFunc = func(r *nflogpb.Receiver, gkey []byte, firingAlerts, resolvedAlerts []uint64) error {
		require.Equal(t, s.recv, r)
		require.Equal(t, []byte{0, 0, 0, 0, 0, 0, 0, 1}, gkey)
